...
1. The Schematron transformation process `pipeline-for-svrl.xsl` from the SchXSLT GitHub project at [https://github.com/schxslt/schxslt](https://github.com/schxslt/schxslt) (`schxslt-1.8.6-xslt-only.zip` release package belongs in the `utilities/schematron/schxslt` subdirectory) interprets the Schematron scripts to create the runtime artefact  `bpc/ZZZ/support/BPC-ZZZ-vX.Y-WWWWWWWW-Data-Integrity-Constraints.xsl` to be imported by the shell invocation stylesheet.
