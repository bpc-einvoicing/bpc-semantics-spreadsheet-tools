...
The "`regression/`" directory contains the Shell invocation script "`regression.sh`" that performs a validation on the one "good" and all of the other "bad" files. The error exit reports are trimmed and stored in the "`results/`" subdirectory. These results can be compared to the identically-named files in the "`expected/`" directory for any introduced discrepancies.

The name of each of the regression test files includes the corresponding North American Business/Group Term identifier for reference.
