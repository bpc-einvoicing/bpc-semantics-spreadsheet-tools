...
<!--PATCHED 2007-09-16 G. Ken Holman
     ...
     appears in 5 occurences of: document($stringFileList)
-->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="stringfile">
          <xsl:value-of
            select="document($stringFileList,document(''))/*/lang[@xml:lang=$ancestorlang][1]/@filename"/>
        </xsl:variable>
        <!-- Get the translated string -->
        <xsl:variable name="str" select="document($stringfile,document($stringFileList,document('')))/strings/str[@name=$stringName]"/>
        <xsl:choose>
          <!-- If the string was found, use it. Cannot test $str, because value could be empty. -->
          <xsl:when test="document($stringfile,document($stringFileList,document('')))/strings/str[@name=$stringName]">
            <xsl:value-of select="$str"/>
          </xsl:when>
          <!-- If the current language is not the default language, try the default -->
          <xsl:when test="$ancestorlang!=$DEFAULTLANG">
            ...
            <xsl:variable name="backupstringfile">
              <xsl:value-of
                select="document($stringFileList,document(''))/*/lang[@xml:lang=$DEFAULTLANG]/@filename"/>
            </xsl:variable>
            <xsl:variable name="str-default"
              select="document($backupstringfile,document($stringFileList,document('')))/strings/str[@name=$stringName]"/>
            <xsl:choose>
              <!-- If a default was found, use it, but warn that fallback was needed.-->
              <xsl:when test="string-length($str-default)>0">
                <xsl:value-of select="$str-default"/>
